...
>>>>>>> Take hoverboard device port from ROS parameters
...
void Hoverboard::protocol_recv (char byte) {
    ...
    if (msg_len == sizeof(SerialFeedback))
    {
        ...
        if (msg.start == START_FRAME && msg.checksum == checksum)
        {
            ...
            f.data = (double)msg.left_dc_curr / 100.0;
            left_curr_pub.publish(f);

            f.data = (double)msg.right_dc_curr / 100.0;
            right_curr_pub.publish(f);
